...
use litebox_common_linux::{
    AddressFamily, AtFlags, EfdFlags, FcntlArg, FileDescriptorFlags, SockType, errno::Errno,
};
...
#[test]
fn test_ftruncate_pipe_and_socket() {
    let task = init_platform(None);
    ...
    for fd in [read_fd, write_fd] {
        let fd = i32::try_from(fd).unwrap();
        assert_eq!(
            task.sys_ftruncate(fd, 0),
            Err(Errno::EINVAL),
            "ftruncate on pipe fd should return EINVAL"
        );
        task.sys_close(fd).expect("Failed to close pipe fd");
    }

    let socket_fd = task
        .sys_socket(AddressFamily::UNIX as u32, SockType::Stream as u32, 0)
        .expect("Failed to create UNIX socket");
    let socket_fd = i32::try_from(socket_fd).unwrap();
    assert_eq!(
        task.sys_ftruncate(socket_fd, 0),
        Err(Errno::EINVAL),
        "ftruncate on socket fd should return EINVAL"
    );
    task.sys_close(socket_fd)
        .expect("Failed to close UNIX socket");
}
...
fn test_getdent64() {
    ...
    let result = task.sys_getdirent64(
        dir_fd,
        MutPtr::from_usize(tiny_buffer.as_mut_ptr() as usize),
        tiny_buffer.len(),
    );
    assert_eq!(
        result,
        Err(Errno::EINVAL),
        "Should return EINVAL if buffer is too small for any dirent entry"
    );
    task.sys_close(dir_fd).expect("Failed to close directory");
    ...
    // Test 5: File descriptor pointing to a regular file (not a directory)
    let file1_fd = task
    ...
    // Test 6: Zero-length buffer should return EINVAL
    let dir_fd = task
        .sys_open("/", OFlags::RDONLY, Mode::empty())
        .expect("Failed to open root directory");
    let dir_fd = dir_fd.try_into().unwrap();
    ...
    // Test 7: Multiple reads (test directory offset tracking)
